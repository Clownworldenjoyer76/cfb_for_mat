...
Source:
  https://site.api.espn.com/apis/site/v2/sports/football/college-football/scoreboard?dates=2026&seasontype=2&groups=80&limit=1000
...
def fetch_scoreboard() -> dict[str, Any] | None:
    url = (
        ...
        f"college-football/scoreboard?dates={YEAR}&seasontype=2"
    ...
    try:
        ...
        log(
            f"WARNING: HTTP error fetching scoreboard: "
            f"{e.code} {e.reason}"
        )
        return None

    except urllib.error.URLError as e:
        log(f"WARNING: URL error fetching scoreboard: {e.reason}")
        return None

    except Exception as e:
        log(f"WARNING: Scoreboard fetch failed: {e}")
        return None
    ...
    if game_timezone:
        try:
            ...
            log(
                ...
                f"game_id={game_id} "
                f"game_timezone={game_timezone} error={e}"
            )
