...
Pulls depth chart data for every CFB team from ESPN's season-specific
core API, reshapes the response to match the original
...
Endpoints used:
    ...
    https://sports.core.api.espn.com/v2/sports/football/leagues/college-football/seasons/{season}/teams/{id}/depthcharts
...
DEPTHCHART_URL_TEMPLATE = "https://sports.core.api.espn.com/v2/sports/football/leagues/college-football/seasons/{season}/teams/{team_id}/depthcharts"
OUTPUT_PATH = "docs/win/football/cfb/data/raw/raw_depth.csv"
...
def build_old_shape(core_response, team_id, team_abbr):
    """
    Converts the core API's
    {items:[{id,name,positions:{pos:{position,athletes:[{rank,athlete:{$ref}}]}}}]}
    shape into the old site API's
    {depthchart:[{id,name,positions:{pos:{position,athletes:[{id,displayName,...}]}}}]}
    shape.
    """
    depthchart = []
    ...
    for item in core_response.get("items", []):
        ...
        for pos_key, pos_val in item.get("positions", {}).items():
            ...
            athletes_sorted = sorted(
                athletes_raw,
                key=lambda a: a.get("rank", 999),
            )

            athletes_out = []

            for athlete_item in athletes_sorted:
                athlete_ref = athlete_item.get("athlete", {})
