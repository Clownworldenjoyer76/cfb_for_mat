#!/usr/bin/env python3
# docs/win/football/cfb/scripts/00_intake/clean_e_pred.py
...
IN_DIR = "docs/win/football/cfb/00_intake/predictions/e_predictions"
OUT_DIR = "docs/win/football/cfb/00_intake/predictions/clean"
LOG_PATH = "docs/win/football/cfb/errors/00_intake/clean_e_pred.txt"
...
def main():
    ...
    log.append(
        "SUMMARY: files_read=%d rows_read=%d games=%d "
        "files_written=%d games_written=%d"
        % (
            len(files),
            rows_read,
            len(games),
...
def process_row(
    ...
    if " at " not in game_name:
        raise ValueError(
            "cannot split game_name %r on ' at '"
            % game_name
        )
    ...
    if gid not in games:
        ...
        if (
            ...
            log.append(
                "MISMATCH %s line %d: game_id %s "
                "season/type/week differs from prior row"
                % (
                    path,
                    lineno,
                    gid,
    ...
    if rec[prefix + "_prob"]:
        log.append(
            "DUPLICATE %s line %d: second %s row "
            "for game_id %s (overwriting)"
            % (
                path,
                lineno,
                side,
    ...
    rec[prefix + "_prob"] = to_decimal_prob(
