#!/usr/bin/env python3
"""
CFB Regression Runner (robust join: alias + normalized team keys)

- Loads data/modeling_dataset.csv
- Finds scores file (prefers SCORES_CSV, else scans data/*games*.csv / *scores*.csv)
- Normalizes team names on BOTH modeling and scores (accents stripped, case/space/punct ignored)
- Applies aliases from mappings/team_aliases.csv if present (to BOTH sides)
- Expands raw home/away scores to per-(game_id, team) when needed
- Strict-joins on (game_id, team_key); logs join stats and scores file used
- Drops rows with NaN in TARGET_COL; logs dropped_NaN_targets
...
- Writes logs_model_regression.txt, data/model_regression_metrics.csv, data/model_regression_coeffs.csv, models/*.pkl
"""
...
SCORES_CSV_ENV = os.environ.get("SCORES_CSV")      # preferred explicit path
...
def apply_alias(series: pd.Series, alias_map: dict[str,str]) -> pd.Series:
    if not alias_map: return series
    key = series.map(std_name)
    mapped = key.map(alias_map)
    # if alias provided, use it; else keep original
    return mapped.where(mapped.notna(), series)

# ---- scores discovery and normalization ----
def find_scores_file() -> str | None:
    ...
    for p in ordered:
        ...
        if {"game_id"}.issubset(cols) and ({"team","points_scored"}.issubset(cols) or {"home_team","away_team"}.issubset(cols)):
            return p
    return None
...
def normalize_scores_to_team_rows(df_scores: pd.DataFrame, alias_map: dict[str,str]) -> pd.DataFrame:
    ...
    # if already per-team with points_scored, just clean + alias
    if gid and team and ps:
        ...
        return out
    # else expand from home/away points
...
def exclude_leaks(df: pd.DataFrame, target_col: str, pattern: str | None) -> tuple[pd.DataFrame,int,list[str]]:
    if not pattern: return df, 0, []
    rx = re.compile(pattern)
    keep, dropped = [], []
    for c in df.columns:
        if c == target_col:
            keep.append(c); continue
        if rx.search(c): dropped.append(c)
        else: keep.append(c)
...
def temporal_split(df: pd.DataFrame, X: pd.DataFrame, y: pd.Series, test_frac=0.2):
    ...
    n = len(ordered); cut = int(n*(1-test_frac))
    return X.loc[ordered[:cut]], y.loc[ordered[:cut]], X.loc[ordered[cut:]], y.loc[ordered[cut:]], ("temporal(season,week)" if {"season","week"}.issubset(df.columns) else "index")

def fit_eval(Xt, yt, Xv, yv, name, pipe):
    pipe.fit(Xt, yt); pred = pipe.predict(Xv)
...
def main():
    ...
    # alias-map and normalized keys for modeling side
    alias_map = load_alias_map(ALIASES_CSV)
    dfm["team_alias"] = apply_alias(dfm["team"], alias_map) if "team" in dfm.columns else dfm["team"]
    dfm["team_key"]   = dfm["team_alias"].map(std_name)

    # find & prep scores
    scores_path = find_scores_file()
    if not scores_path:
        raise FileNotFoundError("No scores file found. Ensure fetch/clean steps produce a CSV under data/ with games/scores.")
    s_raw = pd.read_csv(scores_path)
    s = normalize_scores_to_team_rows(s_raw, alias_map)

    # strict join on (game_id, team_key)
    if "game_id" not in dfm.columns:
        raise KeyError("modeling_dataset missing 'game_id'.")
    merged = dfm.merge(
    ...
    log(f"outcome_join_mode=strict(game_id,team_key)")
    log(f"outcome_merge_missing_rows={miss}")

    # target
    target = select_target(merged, TARGET_COL)
    pre_drop = int(merged[target].isna().sum()); log(f"pre_drop_NaN_targets={pre_drop}")
    ...
    for name, pipe in models.items():
        ...
        log(f"{name}: rmse={rmse:.6f}, mae={mae:.6f}, r2={r2:.6f}, n_train={len(Xtr)}, n_test={len(Xte)}, n_features={Xtr.shape[1]}, split_mode={split_mode}")
        rows_metrics.append({"model": name, "rmse": rmse, "mae": mae, "r2": r2, "n_train": len(Xtr), "n_test": len(Xte), "n_features": Xtr.shape[1], "split_mode": split_mode})
        ...
        try:
            ...
            if hasattr(est, "intercept_"):
                rows_coeffs.append({"model": name, "feature": "__intercept__", "coefficient": float(np.atleast_1d(est.intercept_)[0])})
